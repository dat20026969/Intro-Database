...
where not exists(
	...
	from giaovien gv1 join thamgiadt dt1 on gv1.magv = dt1.magv
...
where not exists(
	...
	where not exists(
		select *
		from giaovien gv1 join thamgiadt dt on gv1.magv = dt.magv
			join detai dt on dt.madt = dt.madt
		where cd.macd = cd.macd
	)
)
...
where not exists(
	...
	from giaovien gv1 join thamgiadt tgdt on gv1.magv = st.magv
	where st.madt = '006' and gv.magv = gv1.magv
	)
)
